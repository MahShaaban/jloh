...
def convert_to_vcf(args, tmp_dir, Snps, all2vcf_exec):
    """
    Convert MUMmer SNPs output to VCF format using custom implementation.
    11/03/2022 - Updated to use Python implementation instead of all2vcf
    """
    snps_A = Snps[0]
    snps_B = Snps[1]
    ...
    # Convert using custom Python implementation
    convert_snps_to_vcf(snps_A, outfile_A, args.ref_A)
    convert_snps_to_vcf(snps_B, outfile_B, args.ref_B)

    return outfile_A, outfile_B


def convert_snps_to_vcf(snps_file, output_vcf, reference_name):
    """
    Convert MUMmer SNPs output to VCF format.
    Custom implementation to replace all2vcf dependency.
    """
    with open(snps_file, 'r') as infile, open(output_vcf, 'w') as outfile:
        # Write VCF header
        outfile.write("##fileformat=VCFv4.2\n")
        outfile.write("##source=JLOH_g2g\n")
        outfile.write(f"##reference={reference_name}\n")
        outfile.write("##INFO=<ID=TYPE,Number=1,Type=String,Description=\"Type of variant\">\n")
        ...
        for line in infile:
            ...
            try:
                # Parse MUMmer SNPs format
                # Format: [P1] [SUB] [P2] | [BUFF] | [DIST] | [R] | [Q] | [LEN R] | [LEN Q] | [FRM] | [TAGS]
                parts = line.split('\t')
                if len(parts) < 5:
                    parts = line.split()
                ...
                if len(parts) >= 4:
                    ...
                    # Try to get chromosome name from the end of the line
                    chrom = parts[-1] if len(parts) > 3 else "chr1"
                    
                    # Clean chromosome name
                    if '|' in chrom:
                        chrom = chrom.split('|')[0]
                    
                    # Skip if bases are the same or contain '.'
                    if ref_base != query_base and '.' not in ref_base and '.' not in query_base:
                        outfile.write(f"{chrom}\t{ref_pos}\t.\t{ref_base}\t{query_base}\t60\tPASS\tTYPE=SNP\n")
